...
import { selectSalesData, selectTotalSales, selectGrowthRate } from "../../redux/slice/salesSlice";
...
const SalesAnalysis = () => {
  ...
  useEffect(() => {
    ...
    // ✅ 최고 매출액 대비 하락률 계산
    /* if (highest.valueHeight > 0) {
      const dropRate = ((highest.valueHeight - lowest.valueHeight) / highest.valueHeight) * 100;
      setLowestDropRate(dropRate.toFixed(2));
    } */
  }, [salesData]);
  ...
  //const maxBaseHeight = Math.max(...salesData.map(data => data.baseHeight)); // baseHeight 최대값 계산
  //const maxValueHeight = Math.max(...salesData.map(data => data.valueHeight));
...
/*   // 👉 정규화된 값 적용
  const normalizedData = salesData.map((data) => ({
    ...data,
    baseHeight: (data.baseHeight / maxBaseHeight) * 100,
    valueHeight: (data.valueHeight / maxValueHeight) * 100,
  }));
 */
      ...
      <div className="flex justify-between items-end mt-10 gap-x-2 w-full">
        {modifiedSalesData.map((data, index) => (
          <div key={data.id} className="flex flex-col items-center w-full">
            {/* 베이스 막대 
            <motion.div
              className="w-full bg-gradient-to-t from-[#e8e8e8] to-[#f6f6f6] rounded-md mb-1"
              style={{
